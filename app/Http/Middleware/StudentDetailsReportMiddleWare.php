...
class StudentDetailsReportMiddleWare implements Middleware {
	...
	public function handle($request, Closure $next)
	{
        ...
        if ( ! Sentry::check())
        {
         ...
         $faculity = $admin = \Sentry::findGroupByName('faculity');
         $finance  = $admin = \Sentry::findGroupByName('finance');
        // Now check to see if the current user has the 'finance' permission
        if (!Sentry::getUser()->inGroup($faculity) !Sentry::getUser()->inGroup($finance))
        {
            if ($request->ajax())
            {
                return response('Unauthorized.', 401);
